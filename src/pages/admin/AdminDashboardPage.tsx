  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1">Overview of your clinic activity</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div key={stat.label} className="card dark:bg-neutral-800 p-5">
      ...
      {/* Recent Appointments */}
      <div className="card dark:bg-neutral-800 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 dark:border-neutral-700">
          ...
          <a href="/admin/appointments" className="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium">
            View all →
          </a>
        </div>
        ...
        {appointments.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-neutral-400 dark:text-neutral-500">No appointments yet</p>
          </div>
        ) : (
          <div className="divide-y divide-neutral-50 dark:divide-neutral-700">
            {appointments.slice(0, 8).map((appt) => (
              <div key={appt.id} className="px-6 py-4 flex items-center gap-4 hover:bg-neutral-50 dark:hover:bg-neutral-700/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-neutral-900 dark:text-neutral-100 text-sm truncate">{appt.patient_name}</p>
      ...
      <div className="card dark:bg-neutral-800 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 dark:border-neutral-700">
          ...
          <a href="/admin/messages" className="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium">
            View all →
          </a>
        </div>
        ...
        {messages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-neutral-400 dark:text-neutral-500">No messages yet</p>
          </div>
        ) : (
          <div className="divide-y divide-neutral-50 dark:divide-neutral-700">
            {messages.slice(0, 5).map((msg) => (
              <div key={msg.id} className={`px-6 py-4 flex items-start gap-4 hover:bg-neutral-50 dark:hover:bg-neutral-700/50 transition-colors ${!msg.is_read ? 'bg-primary-50/50 dark:bg-primary-900/10' : ''}`}>
                <div className="w-9 h-9 rounded-full bg-teal-100 dark:bg-teal-900/30 flex items-center justify-center text-teal-700 dark:text-teal-300 font-bold text-sm flex-shrink-0 mt-0.5">
                  {msg.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-neutral-900 dark:text-neutral-100 text-sm">{msg.name}</p>
                    {!msg.is_read && <span className="w-2 h-2 rounded-full bg-primary-500 dark:bg-primary-400 flex-shrink-0" />}
                  </div>
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 truncate">{msg.subject || msg.message}</p>
                </div>
