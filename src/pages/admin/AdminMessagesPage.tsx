          ...
          <div className="divide-y divide-neutral-50">
            {filtered.length === 0 ? (
              ...
              filtered.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex items-start gap-4 px-6 py-4 hover:bg-neutral-50 transition-colors cursor-pointer ${!msg.is_read ? 'bg-primary-50/40' : ''}`}
                  onClick={() => openMessage(msg)}
                >
                  <div className="w-10 h-10 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-sm flex-shrink-0 mt-0.5">
                    {msg.name.charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className={`text-sm ${!msg.is_read ? 'font-bold text-neutral-900' : 'font-medium text-neutral-700'}`}>
                        {msg.name}
                      </p>
                      {!msg.is_read && <span className="w-2 h-2 rounded-full bg-primary-500 flex-shrink-0" />}
                    </div>
                    <p className="text-xs text-neutral-400 mb-0.5">{msg.email}</p>
                    {msg.subject && <p className="text-sm text-neutral-600 font-medium truncate">{msg.subject}</p>}
                    <p className="text-xs text-neutral-400 truncate">{msg.message}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs text-neutral-400">{new Date(msg.created_at).toLocaleDateString()}</span>
                    <button
          ...
          <div className="p-6 space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-bold text-neutral-900">{selected.name}</p>
                  <p className="text-neutral-500">{new Date(selected.created_at).toLocaleString()}</p>
                </div>
              </div>
            ...
            {selected.subject && (
              <div>
                <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-1">Subject</p>
                <p className="font-semibold text-neutral-800">{selected.subject}</p>
              </div>
            )}

            <div>
              <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Message</p>
              <div className="bg-neutral-50 rounded-xl p-4 text-neutral-700 text-sm leading-relaxed whitespace-pre-wrap">
                {selected.message}
              </div>
            </div>
